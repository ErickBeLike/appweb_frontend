...
export class EmpleadosRegistroComponent implements OnInit {
    ...
    constructor(
        ...
        this.formEmpleado = this.fb.group({
            ...
            SABADO: [false],
            DOMINGO: [false],
            horarioEntradaHora: ['', [Validators.required, Validators.pattern(/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/)]],
...
prepareSaveEmpleado() {
  ...
  const diasLaborales = Object.keys(formModel)
    .filter(key => key !== 'nombre' && key !== 'apellidoPaterno' && key !== 'apellidoMaterno' && key !== 'idCargo'
      && key !== 'fechaNacimiento' && key !== 'sexo' && key !== 'correo' && key !== 'telefono' && key !== 'direccionEmpleado'
       && key !== 'horarioEntradaHora' && key !== 'horarioEntradaAMPM' && key !== 'horarioSalidaHora' && key !== 'horarioSalidaAMPM'
       && formModel[key]);
      ...
      return {
          ...
          horarioEntrada: formModel.horarioEntradaHora + ' ' + formModel.horarioEntradaAMPM,
          horarioSalida: formModel.horarioSalidaHora + ' ' + formModel.horarioSalidaAMPM
      };
  }
    ...
    onInputPa(event: any) {
        const inputValue = event.target.value;
        const newValue = inputValue.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ\s]/g, ''); 
        this.formEmpleado.get('apellidoPaterno')?.setValue(newValue, { emitEvent: false });
    }

    onInputMa(event: any) {
        const inputValue = event.target.value;
        const newValue = inputValue.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ\s]/g, ''); 
        this.formEmpleado.get('apellidoMaterno')?.setValue(newValue, { emitEvent: false });
    }
    ...
    formatearHorario(event: any): void {
        ...
        if (valor.length >= 1 && valor.length <= 2) {
            ...
            if (valor.length === 2 && valor.charAt(0) === '2' && parseInt(valor.charAt(1), 10) > 3) {
